...
def compare_models(models_configs: dict, num_episodes: int = 100,
    ...
    for name, (model_path, config_path) in models_configs.items():
        ...
        results = evaluate_model(model_path, config_path, num_episodes, env_type)
...
def main():
    ...
    parser.add_argument('--output', type=str, default=None,
                        help='Path to save evaluation results (default: runs/val/evaluation_YYYYMMDD_HHMMSS.json)')

    args = parser.parse_args()

    # Set default output path if not provided
    if args.output is None:
        # Create runs/val directory if it doesn't exist
        output_dir = Path("runs/val")
        output_dir.mkdir(parents=True, exist_ok=True)

        # Generate filename with timestamp
        import datetime
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        args.output = output_dir / f"evaluation_{timestamp}.json"
